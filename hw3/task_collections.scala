...
object task_collections {
  ...
  def capitalizeIgnoringASCII(text: List[String]): List[String] = {
    text
      .zipWithIndex
      .collect({case x if (isASCIIString(x._1) && x._2 == 0) => x._1
      case x if (isASCIIString(x._1) && x._2 > 0) => x._1.toUpperCase
      case x if (!isASCIIString(x._1) && x._2 == 0) => x._1
      case x if (!isASCIIString(x._1) && x._2 > 0) => x._1.toLowerCase} )
  }
  ...
  def numbersToNumericString(text: String): String = {
    val mapForReplace = Map(
      "0" -> "zero", "1" -> "one",
      "2" -> "two", "3" -> "three",
      "4" -> "four", "5" -> "five",
      "6" -> "six", "7" -> "seven",
      "8" -> "eight", "9" -> "nine")
    mapForReplace.foldLeft(text) {
      case (updatingString, (key, mapString)) => updatingString.replaceAll(key, mapString)
    }
  }
  ...
  def intersectionAuto(dealerOne: Iterable[Auto], dealerTwo: Iterable[Auto]): Iterable[Auto] = {
    dealerOne.toSet.concat(dealerTwo.toSet)
  }
