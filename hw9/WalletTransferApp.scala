...
object WalletTransferApp extends IOApp.Simple {
  ...
  final class InMemWallet[F[_]:Concurrent](ref: Ref[F, BigDecimal]) extends Wallet[F] {
    ...
    def withdraw(amount: BigDecimal): F[Either[WalletError, Unit]] =
    {
      ref.updateAndGet(_ - amount)
        ...
        .attempt.map(_.leftMap[WalletError](error => BalanceTooLow).void)
    }
  }
  ...
  def wallet(balance: BigDecimal): IO[Wallet[IO]] = {
    val cache: IO[Ref[IO, BigDecimal]] = Ref.of[IO, BigDecimal](balance)
    for {
      ref <- cache
    } yield new Wallet[IO] {

      override def balance: IO[BigDecimal] = ref.get

      override def topup(amount: BigDecimal): IO[Unit] = ref.update(_ + amount)

      override def withdraw(amount: BigDecimal): IO[Either[WalletError, Unit]] = {
        ref.updateAndGet(_ - amount)
          .ensure(new RuntimeException())(x => {
          x > 0
        })
